...
export async function signString(message: string, key: Uint8Array): Promise<Signature> {
...
export async function signUint8Array(payload: Uint8Array, key: Uint8Array): Promise<Signature> {
  const hashBuffer = await crypto.subtle.digest("SHA-256", payload);
  const ret = ecdsaSign(new Uint8Array(hashBuffer), key, {});
  return ret;
}
